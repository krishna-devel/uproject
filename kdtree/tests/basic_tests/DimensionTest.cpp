...
TEST_F(DimensionTest, getValuesAlongDimension_checks_for_valid_index_size) {
    Segment<double, int> adapter1 (dMatrix);
    Dimension<double, int> dimension (adapter1, 5);
    ASSERT_EQ(0, dimension.getValuesAlongDimension().size());
}

TEST_F(DimensionTest, getValuesAlongDimension_returns_right_values) {
    Segment<double, int> adapter1 (dMatrix);
    Dimension<double, int> dimension1 (adapter1, 0);
    const vector<ValueAlongDimension<double, int>> valuesAlongDimension = dimension1.getValuesAlongDimension();
...
TEST_F(DimensionTest, getValuesAlongDimension_returns_right_values_for_partial_row_indices) {
    ...
    Segment<double, int> adapter2 (dMatrix, indices);
    Dimension<double, int> dimension2 (adapter2, 1);

    const vector<ValueAlongDimension<double, int>> valuesAlongDimension = dimension2.getValuesAlongDimension();
