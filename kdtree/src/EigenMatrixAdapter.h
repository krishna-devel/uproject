...
public:
    // constructors
    EigenMatrixAdapter() {};
    EigenMatrixAdapter(const Matrix<T, Dynamic, Dynamic> &matrix);
    EigenMatrixAdapter(const Matrix<T, Dynamic, Dynamic> &matrix, const vector<int> &indices);
...
template <typename T>
EigenMatrixAdapter<T>::EigenMatrixAdapter(const Matrix<T, Dynamic, Dynamic> &matrix) : samples(matrix) {
    rowIndices = vector<int>(matrix.rows());
    iota(begin(rowIndices), end(rowIndices), 0);
}

template <typename T>
EigenMatrixAdapter<T>::EigenMatrixAdapter(
